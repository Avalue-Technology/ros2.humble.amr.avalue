...
static uint32_t utf8ToUtf32(const char* str, size_t len)
{
	...
	if (len >= 1 && (str[0] & 0x80) == 0) { // ASCII�ַ�
		codepoint = str[0];
	} else if (len >= 2 && (str[0] & 0xE0) == 0xC0) { // 2�ֽ�UTF-8�ַ�
		codepoint = ((str[0] & 0x1F) << 6) | (str[1] & 0x3F);
	} else if (len >= 3 && (str[0] & 0xF0) == 0xE0) { // 3�ֽ�UTF-8�ַ�
		codepoint = ((str[0] & 0x0F) << 12) | ((str[1] & 0x3F) << 6) | (str[2] & 0x3F);
	} else if (len >= 4 && (str[0] & 0xF8) == 0xF0) { // 4�ֽ�UTF-8�ַ�
...
std::wstring ConvertUtil::utf8ToWstring(const std::string& src)
{
	...
	while (i < src.length()) {
		size_t len = 1;
		while (i + len < src.length() && (src[i + len] & 0xC0) == 0x80) { // ����UTF-8�ַ�����
			len++;
		}
		uint32_t codepoint = utf8ToUtf32(&src[i], len); // ��UTF-8�ַ�ת��ΪUnicode���
		wchar_t wc = static_cast<wchar_t>(codepoint); // ��Unicode���ת��Ϊ���ַ�
		wstr += wc; // �����ַ����ӵ����ַ�����
		i += len; // �ƶ�����һ��UTF-8�ַ�
	}
	return wstr;
