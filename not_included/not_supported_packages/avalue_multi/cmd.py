...
def control_onecar(carnub,carip,carcmd):
    ...
    if carcmd == 'open' and ip_status[carnub-1] == "stop" and carnub == 1:
        command.send("DISPLAY=:0 bash ~/avalue_ros2/src/avalue_multi/scripts/leadercar.sh \n")
        time.sleep(6)
        ip_status[carnub-1] = "open"
    elif carcmd == 'open' and ip_status[carnub-1] == "stop" and carnub != 1:
        command.send("DISPLAY=:0 bash ~/avalue_ros2/src/avalue_multi/scripts/slavecar.sh \n")
        time.sleep(6)
        ip_status[carnub-1] = "open"
    elif carcmd == 'stop':
        command.send("DISPLAY=:0 bash ~/avalue_ros2/src/avalue_multi/scripts/killall.sh \n")
        time.sleep(6)
