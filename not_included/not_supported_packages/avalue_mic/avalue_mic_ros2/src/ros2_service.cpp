/****************************************************************/
/* Copyright (c) 2023 avalue Technology, Inc   				*/
/* function:Serial interface service							*/
/* 功能：串口交互服务												*/
/****************************************************************/
#include "avalue_mic.h"

/********************************************************
Function: Initialize service
功能: 初始化服务
*********************************************************/
void avalue_Mic::setupMicArrayServices()
{
	get_device_srv_ = this->create_service<avalue_mic_msg::srv::GetDeviceType>(
		"get_device_info",[this](const std::shared_ptr<avalue_mic_msg::srv::GetDeviceType::Request> request,
								std::shared_ptr<avalue_mic_msg::srv::GetDeviceType::Response> response){
		response->success = getDeviceTypeCallback(request,response);
		});
	set_awake_word_srv_ = this->create_service<avalue_mic_msg::srv::SetAwakeWord>(
		"set_awake_word",[this](const std::shared_ptr<avalue_mic_msg::srv::SetAwakeWord::Request> request,
								std::shared_ptr<avalue_mic_msg::srv::SetAwakeWord::Response> response){
		response->success = setAwakeWordCallback(request,response);
		if(response->success) exit(1);
		});
	switch_mic_srv_ = this->create_service<avalue_mic_msg::srv::SwitchMic>(
		"switch_mic_type",[this](const std::shared_ptr<avalue_mic_msg::srv::SwitchMic::Request> request,
								std::shared_ptr<avalue_mic_msg::srv::SwitchMic::Response> response){
...
功能: 获取设备版本信息
*********************************************************/
bool avalue_Mic::getDeviceTypeCallback(const std::shared_ptr<avalue_mic_msg::srv::GetDeviceType::Request>& request,
								std::shared_ptr<avalue_mic_msg::srv::GetDeviceType::Response>& response){
...
*********************************************************/
bool avalue_Mic::setAwakeWordCallback(const std::shared_ptr<avalue_mic_msg::srv::SetAwakeWord::Request>& request,
	                           std::shared_ptr<avalue_mic_msg::srv::SetAwakeWord::Response>& response){
...
功能: 切换麦克风类型
*********************************************************/
bool avalue_Mic::switchMicCallback(const std::shared_ptr<avalue_mic_msg::srv::SwitchMic::Request>& request,
								std::shared_ptr<avalue_mic_msg::srv::SwitchMic::Response>& response){
...
功能: 下发数据包
*********************************************************/
bool avalue_Mic::Send_Serial_Data(ServicePacket &pkg)
{
	string section;
...
功能: 制作数据包
*********************************************************/
string avalue_Mic::MakeMsgPacket(unsigned short sid, MsgType type, const string &content)
{
