...
def generate_robot_node(robot_urdf,child):
    return launch_ros.actions.Node(
        ...
        arguments=[os.path.join(get_package_share_directory('avalue_robot_urdf'), 'urdf', robot_urdf)],
    )
...
def generate_launch_description():
    ...
    mini_omni = LaunchConfiguration('mini_omni', default='false')
    ...
    mini_omni_ = GroupAction(
        condition=IfCondition(mini_omni),
        actions=[
            generate_robot_node('mini_omni_robot.urdf','mini_omni'),
            generate_static_transform_publisher_node(['0.015', '0', '0.17'], ['0', '0', '0'], 'base_footprint', 'laser'),
            generate_static_transform_publisher_node(['0.075', '0.12', '0'], ['0', '0', '0'], 'base_footprint', 'camera_link'),
    ])   
