...
using namespace std;
class avalue_joy : public rclcpp::Node
{
...
: Node ("avalue_joy")
{
	RCLCPP_INFO(this->get_logger(),"avalue_joy111");
   //读取参数服务器中的变量值
	 flag_mec=0;
   ...
   this->get_parameter("v_angular", v_angular);
	...
	sub =  this->create_subscription<sensor_msgs::msg::Joy>("/joy", 10, std::bind(&avalue_joy::callback, this, _1)); //订阅手柄发来的数据
...
int main(int argc, char * argv[]){
	...
	printf("joy_control\n");
	rclcpp::spin(std::make_shared<avalue_joy>());
	return 0;
}
